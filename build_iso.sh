...
if [ "$1" == "--custom" ]; then
  ...
  addFlagCombination() {
    listFlags=("french" "FRENCH rather than english" "OFF" "dbg" "DEBUG packages" "OFF" "raid" "lvm RAID" "ON" "cockpit" "COCKPIT packages" "OFF" "kerberos" "KERBEROS packages" "OFF" "cluster" "CLUSTER rather than standalone" "ON")
    arrVar=()
